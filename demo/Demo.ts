...
export class CustomValidityDemo extends LitElement {
  ...
  render() {
    ...
    <div class="form">
      ...
      <formsey-form id="right" src="https://www.formsey.com/live/GICKa9Zi7VFBEuWofBC4" @load="${e => { this.formNavigator.components = this.form.components; this.formNavigator.definition = this.form.definition; this.formNavigator.value = this.form.value }}" @change="${e => { console.log("Value changed"); this.formNavigator.value = e.detail.value }}" @focus="${this.f}" @invalid="${(e: InvalidEvent) => this.formNavigator.errors = e.detail}"></formsey-form>
    </div></fs-splitter>`
  }
