...
registerIcon("End", html`<fs-icon><svg viewBox="0 0 32 32"><path d="M24 4v24h-4v-11l-10 10v-22l10 10v-11z"></path></svg></fs-icon>`)
...
registerFormatter("columns", {
  ...
  containerStyle(layout: ColumnsLayout): string {
    ...
    return `display:grid;grid-template-columns:${layout.columns?.map((column: any) => `minmax(0,${typeof column == "number" ? column : column.width}fr)`).join(" ") || "1fr"};gap:${gaps}`
  },
  fieldStyle(layout: ColumnsLayout, field: FieldDefinition, fields: FieldDefinition[], index: number): string {
    const horizontal = (<any>layout.columns?.[index % layout.columns.length])?.horizontal
