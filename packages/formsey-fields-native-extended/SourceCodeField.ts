...
export class SourceCodeField extends LabeledField<SourceCodeFieldDefinition, string> {
  ...
  protected renderField() {
    return html`<fs-ace class="input" style="min-height:${ifDefined(this.definition.height)}" .value=${this.value} ?gutter="${this.definition.gutter}" mode="${ifDefined(this.definition.mode)}" theme="${ifDefined(this.definition.theme)}" ?readonly="${this.definition.readonly}" @change=${this.changed} @focus="${this.focused}" @blur="${this.blurred}"></fs-ace>`;
  }
