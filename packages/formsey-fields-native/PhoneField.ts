...
getLibrary("native").registerComponent("phone", {
  importPath: "@formsey/fields-native/PhoneField",
    ...
    return html`<formsey-phone id="${ifDefined(id)}" .components=${components} .settings=${settings} .definition=${definition} .context=${context} .value=${value} .parentPath=${parentPath} .errors=${errors} @change="${changeHandler}" @input="${changeHandler}" @inputChange="${changeHandler}" @invalid=${invalidHandler}></formsey-phone>`
  }
})