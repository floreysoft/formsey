...
getLibrary("native").registerComponent("phone", {
  importPath: "@formsey/fields-native/PhoneField",
    ...
    return html`<formsey-phone id="${ifDefined(id)}" .library=${library} .settings=${settings} .definition=${definition as any} .context=${context} .value=${value} .parentPath=${parentPath} .errors=${errors} @change="${changeHandler}" @input="${changeHandler}" @inputChange="${changeHandler}" @invalid=${invalidHandler}></formsey-phone>`
  }
})