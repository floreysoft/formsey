...
export class SelectField extends LabeledField<ListFieldDefinition, string> {
  ...
  renderField() {
    ...
    const button = createField({ library: this.library, context: this.context, settings: this.settings, definition: { type: "button", name: "value", icon: selectedOption?.icon, text: selectedOption?.label } as StringFieldDefinition, parentPath: this.path(), errors: this.errors, changeHandler: (event: ValueChangedEvent<any>) => this.search(event) })
