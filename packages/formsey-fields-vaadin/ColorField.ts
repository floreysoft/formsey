...
getLibrary("vaadin").registerComponent("color", {
  importPath: "@formsey/fields-vaadin/ColorField",
    ...
    return html`<formsey-color-vaadin id="${ifDefined(id)}" .library=${library} .settings=${settings} .definition=${definition} .context=${context} .value=${value} .parentPath=${parentPath} .errors=${errors} @change="${changeHandler}" @input="${changeHandler}" @inputChange="${changeHandler}" @invalid=${invalidHandler}></formsey-color-vaadin>`
  }
})