...
export class NumberField extends VaadinField<NumberFieldDefinition, string> {
  ...
  renderField() {
    ...
    return html`<vaadin-number-field style="display:flex;width:100%" label="${ifDefined(this.definition.label)}" has-controls ?autofocus="${this.definition.autofocus}" ?required="${ifDefined(this.definition.required)}" ?autocomplete="${ifDefined(this.definition.autocomplete)}" @change="${this.changed}" @input="${this.changed}" name="${this.definition.name}" min="${ifDefined(this.definition.min)}" max="${ifDefined(this.definition.max)}" step="${ifDefined(this.definition.step)}" error-message="${ifDefined(customValidity)}" ?disabled="${ifDefined(this.definition.disabled)}" preventinvalidinput="true" .value="${ifDefined(this.value)}">`
  }
