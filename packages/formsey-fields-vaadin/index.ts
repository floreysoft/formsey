...
if (vaadinLibrary) {
  ...
  vaadinLibrary.settingsEditor = {
    ...
    vaadinLibrary.onSettingsChanged = (settings: Settings): Settings => {
      const lumoStylesheet = (<any>settings)['lumoTheme']
      if (lumoStylesheet) {
        const themes = lumoStylesheet.match(/[^{\}]+(?=})/g)
        if (themes.length == 2) {
          (<any>settings)['light'] = createCustomProperties(themes[0]);
          (<any>settings)['dark'] = createCustomProperties(themes[1]);
        }
      }
      delete (<any>settings)['lumoTheme']
      return settings
    }
}